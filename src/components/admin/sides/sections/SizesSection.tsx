...
const DEFAULT_SIZES: Record<string, Array<{ name: string; weight: string }>> = {
  'fries': [
    { name: 'Petite', weight: '100g' },
    { name: 'Moyenne', weight: '150g' },
    { name: 'Grande', weight: '200g' }
  ],
  'wings': [
    { name: '6 pièces', weight: '180g' },
    { name: '12 pièces', weight: '360g' },
    { name: '24 pièces', weight: '720g' }
  ],
  'onion_rings': [
    { name: 'Regular', weight: '150g' },
    { name: 'Large', weight: '250g' }
  ],
  'salad': [
    { name: 'Petite', weight: '200g' },
    { name: 'Grande', weight: '350g' }
  ],
  'coleslaw': [
    { name: 'Regular', weight: '150g' },
    { name: 'Large', weight: '250g' }
...
export default function SizesSection({ form }: SizesSectionProps) {
  ...
  const addDefaultSizes = () => {
    ...
    const defaultSizes = DEFAULT_SIZES[category].map((size, index) => ({
      name: size.name,
      price: index * 1.5,
      weight: size.weight,
      isDefault: index === 0
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              ...
              <div>
                <Input
                  label="Poids"
                  {...register(`sizes.${index}.weight`)}
                  error={errors.sizes?.[index]?.weight?.message}
                  placeholder="ex: 150g"
                />
              </div>
